...
6. **k8s/configmap.yaml** - Kubernetes ConfigMap storing AppDynamics controller configuration
7. **k8s/secret.yaml** - Kubernetes Secret storing sensitive AppDynamics access key credentials
8. **k8s/deployment.yaml** - Kubernetes Deployment orchestrating init container to attach agent, then runs the application
9. **k8s/service.yaml** - Kubernetes Service exposing the application on port 3000
...
Modify Deployment/ ConfigMap → Create Secret → Deploy application → Expose with Service.
