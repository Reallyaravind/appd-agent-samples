# Python Agent APM - Init Container Installation

Here we are using an Init container based approach to instrument a Python application.

## Overview

- We use the init container approach to instrument and detect the Python application
...
4. **secret.yaml** - Kubernetes Secret for storing sensitive AppDynamics credentials
5. **deployment.yaml** - Kubernetes Deployment configuration for running the Python agent in containers
6. **service.yaml** - Kubernetes Service to expose the application on port 8080

### Quick Start Guide

1. **Build the Docker image:** Create the `init-python-app` image using the provided Dockerfile.
2. **Configure AppDynamics variables:** You can provide variables in two ways:
    - **Environment Variables** (used in this application)
    - **ConfigMap** 
3. **Update deployment:** Edit `k8s/deployment.yaml` with your AppDynamics controller details.
4. **Update Secret:** Edit `k8s/secret.yaml` with your access key.
5. **Build the Docker image:**  
    ```sh
    docker build -t init-python-app .
    ```
6. **Apply Kubernetes manifests:**  
    - Build Docker image → Create Secret → Deploy application file → Expose with Service.
    - Apply all manifests:
      ```sh
...
- **Install Agent:** [Splunk AppDynamics Python Agent Installation](https://help.splunk.com/en/appdynamics-saas/application-performance-monitoring/26.4.0/install-app-server-agents/python-agent/install-the-python-agent-in-containers/deploy-the-python-agent-in-the-application-container#InstallthePythonAgentinContainers-python-agentDeploythePythonAgentintheApplicationContainer--en__InstallthePythonAgentinContainers-initUseInitContainers)
